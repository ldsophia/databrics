...
```mermaid
flowchart LR
    A[Changes completed in DEV] --> B[Create or update saved Assurance deployment flow]
    B --> C[Assurance review and/or Harness approval gate]
    C --> D[Harness logs in to DEV IDMC]
    D --> E[Start DEV to UAT deployment through Assurance API v3]
    E --> F[Poll Assurance job details]
    F -->|Success| G[Run UAT functional and regression tests]
    F -->|Partial or Failed| H[Investigate and fix in DEV]
    H --> B
    G -->|Approved| I[Locate exact incoming changelist in UAT Assurance Service]
    I --> J[Harness logs in to UAT IDMC]
    J --> K[Deploy exact changelist from UAT to PROD by flow_id]
    K --> L[Poll job and execute production validation]
    L -->|Issue| M[Rollback supported assets or execute forward recovery]
```
